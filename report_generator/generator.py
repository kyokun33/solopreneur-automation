...
def detect_domain(title: str, features: str) -> str:
    ...
    if any(k in text for k in ["펫푸드", "푸드", "식품", "구독", "정기구독", "간식", "사료", "영양제"]):
        return "petfood_sub"
    elif any(k in text for k in ["반려동물", "펫", "강아지", "고양이", "친환경", "소재", "용품", "브랜드", "제조", "공장", "제품", "키트", "장비", "하드웨어", "부품", "설비"]):
        return "hardware"
    elif any(k in text for k in ["커피", "카페", "식당", "음료", "로봇", "서빙", "음식", "베이커리", "매장", "디저트", "외식", "무인"]):
        return "fnb"
...
| :--- | :--- | :--- | :--- | :--- |
| **1** | 수동 제어 제조 시스템 | 파라미터 수동 세팅 | **맞춤 알고리즘 제조/급여** | 독립 청구항 구성으로 100% 회피 |
| **2** | 단일 데이터 파이프라인 | 유선 통신 기반 제어 | **SSL 256-bit 클라우드 분산 제어** | 기술 독창성 확보 및 특허 출원 |
| **3** | 정기 결제 및 주문 시스템 | 단순 결제 프로세스 | **1회용 인증 키 및 소멸 CS 추적** | 정기구독 독점 권리 확보 |
...
- 해외 표준 기술 규격 사전 검증 테스트베드 구축

### 8. 연구개발 인프라 구축 및 연구인력 유지 관리 방안
- 석/박사급 기술 전담 연구원 상주 개발 및 워크숍
- 주간 코드 리뷰 및 CI/CD 자동화 기술 부채 방지
...
- **기존 경쟁 강도**: 수동 대행업체 난립하나 데이터 기반 맞춤 정기구독 솔루션 부재
...
- **Plan B (경쟁 심화 시)**: B2B 전용 요금제 출시 및 수수료 15% 인하 대응
- **Plan C (시장 침체 시)**: 핵심 린(Lean) 서비스로 축소하여 서버/운영비 최소화

### 7. 마케팅 유저 획득(CAC) 및 LTV(고객생애가치) 재무 추정
- **목표 유저 획득 비용 (CAC)**: 건당 15,000원 이하 유지
- **고객 생애 가치 (LTV)**: 유저당 평균 180,000원 (LTV/CAC 비율 12배 달성)"""

    if domain == "petfood_sub":
        domain_name = "펫푸드 · 바이오헬스 · 식품제조 · 정기구독"
...
- **구독 유지율(Retention) 85% 확보**: 1:1 건강 일지 트래킹 서비스로 고객 LTV 극대화"""
        prob_text_1 = f"현재 국내 펫푸드 시장은 기성 일괄 사료의 영양 불균형과 첨가물 논란, 비싼 수입 렌더링 제품(평균 월 15~20만 원)으로 인해 **{req.target_customer}** 고객층의 접근성 한계와 선택의 어려움을 초래하고 있습니다."
        ...
        comp_table = f"""| 구분 | 기존 수입 기성 사료 | {req.title} (본 프로젝트) | 개선 효과 (수치) |
| :--- | :--- | :--- | :--- |
...
| **구독 단가** | 월 15~20만 원 (고비용) | **자체 생산/직배송으로 합리적 가격** | **비용 40%↓ 절감** |"""
        service_struct = "[반려동물 건강데이터 입력] -> [AI 맞춤 펫푸드 레시피 생성] -> [신선 조리 및 정기배송]"
...
| **3년 차 (2028년)** | 구독 유저 20,000명 | **200,000,000** | **2,400,000,000** | **52%** |"""
        ...
        domain_name = "친환경 · 용품제조 · 하드웨어 · 펫헬스케어"
        focus_points = """- **친환경 소재 및 무독성 안정성 검증**: 친환경 무독성 소재 사용 및 KC/FDA 안전 인증으로 제품 신뢰도 극대화
- **생산 단가 절감 및 자동화 모듈 설계**: 대량 양산형 친환경 몰드 설계로 기존 수입 제품 대비 원가 50% 절감
...
- **유통 채널 다각화**: 네이버, 쿠팡, 자사몰, 11번가 및 B2B 도매 채널 파이프라인 연동"""
        ...
        prob_text_2 = f"특히 상품 사입에 수천만 원의 자금이 묶여 초기 자본이 부족한 창업가들이 폐업하는 비율이 60%에 달합니다. **{req.title}** 프로젝트는 무재고 자동화 유통 구조로 리스크를 없애줍니다."
