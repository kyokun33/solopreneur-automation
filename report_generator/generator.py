...
def build_pdf_file(req: ReportRequest, pdf_path: str):
    ...
    if prog_type == "rnd_25p":
        h1_size, h1_lead, h1_before, h1_after = 18, 24, 24, 12
        h2_size, h2_lead, h2_before, h2_after = 14, 19, 18, 8
        h3_size, h3_lead, h3_before, h3_after = 12, 16, 12, 5
        body_after = 10
        spacer_height = 12
        table_padding = 6
    ...
    for line in lines:
                    ...
                    for r_idx, row in enumerate(raw_rows):
                        ...
                        for cell in row:
                            ...
                            st_cell = ParagraphStyle(
                                'Cell_PDF_Normal', fontName=BOLD_FONT if r_idx == 0 else MAIN_FONT, 
                ...
                if raw_rows:
                    ...
                    for r_idx, row in enumerate(raw_rows):
                        ...
                        for cell in row:
                            clean_cell = cell.replace("**", "").replace("*", "").replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
                            st_cell = ParagraphStyle(
                                'Cell_PDF_App_R', fontName=BOLD_FONT if r_idx == 0 else MAIN_FONT, 
        ...
        if stripped.startswith("# "):
            ...
            target_list.append(Paragraph(clean_title, h1_style))
        elif stripped.startswith("## "):
            target_list = appendix_story if in_appendix else story
            clean_title = stripped[3:].replace("**", "").replace("*", "")
            target_list.append(Paragraph(clean_title, h2_style))
        elif stripped.startswith("### "):
            target_list = appendix_story if in_appendix else story
            clean_title = stripped[4:].replace("**", "").replace("*", "")
            target_list.append(Paragraph(clean_title, h3_style))
        elif stripped.startswith("---"):
            target_list = appendix_story if in_appendix else story
