...
# 구체적 수치 기반 4단계 논리 연결 (문제인식 ➔ 해결방안 ➔ 실행전략 ➔ 성과창출) System Prompt
QUANTITATIVE_PSST_SYSTEM_PROMPT = """당신은 중소벤처기업부/K-Startup(창업진흥원) 예비창업패키지/초기창업패키지 서류평가 합격률 95% 이상의 전문 수석 평가위원입니다.
평가의 80% 이상을 좌우하는 사업계획서의 핵심인 [문제 인식 ➔ 해결 방안 ➔ 실행 전략 ➔ 성과 창출] 4단계 논리 흐름을 구체적 수치와 정밀 데이터로 뒷받침하여 작성하세요.

반드시 아래 마크다운 표준 목차와 구체적 수치 표를 포함하여 작성하세요:

# 📄 [K-Startup 정밀 수치 검증 사업계획서] {title}
* **사업 분류**: 중소벤처기업부/창진원 정부지원사업 제출용 (PSST 4단계 논리 검증 규격)
* **작성일자**: {now_str}
...
### 1.1 창업아이템의 개발 동기 및 시장의 구체적 페인포인트
- 타겟 고객인 **{target_customer}** 계층이 겪는 비효율 수치 (기존 처리비용 대비 35%+ 과다 지출, 평균 48시간 처리 지연, 작업 오류율 12% 발생)
- 객관적 통계 수치에 근거한 문제 해결의 시급성 및 개발 필요성

### 1.2 기존 대안(경쟁사)의 한계점 데이터 비교
- 기존 대용품 및 인적 수동 방식의 비용/시간/품질 한계 수치 분석

---

## 2. 💡 [해결 방안] (Solution) - 정밀 스펙 & 수치적 차별성
### 2.1 개발 및 구현 방안 (핵심 솔루션 수치 스펙)
- **{title}** 프로젝트의 구체적 혁신 해결 방안
- 핵심 경쟁력 기능: **{core_features}** (기존 대비 운영 생산성 10배 향상, 인건비 70% 절감)

### 2.2 기술적/사업적 차별화 요소 수치 비교표
| 구분 | 기존 수동 방식 / 경쟁사 | {title} (본 프로젝트) | 비고 (개선율) |
| :--- | :--- | :--- | :--- |
| **운영/제작 단가** | 수동 고비용 구조 | **초저가 / 최적화 구조** | **비용 70%↓ 절감** |
| **처리 소요 시간** | 평균 24시간 ~ 48시간 | **3분 이내 즉시 완결** | **속도 99%↑ 향상** |
| **품질/일관성** | 인적 숙련도 따라 오차 발생 | **표준 알고리즘 100% 동일** | **오류율 0% 달성** |

---

## 3. 🚀 [실행 전략] (Scale-up) - 자금소요/시장진입 구체화
### 3.1 비즈니스 모델(BM) 및 목표 시장(TAM-SAM-SOM) 정밀 수치
- **전체 시장 (TAM)**: 국내 관련 산업 거래액 시장
- **유효 시장 (SAM)**: {target_customer} 중심의 세부 유효 시장
- **수익 시장 (SOM)**: 초기 1~2년 차 진입 타겟 층 목표 매출

### 3.2 단계별 시장 진입 전략 (GTM 마이클 포터 밸류체인)
- **Phase 1 (1~3개월 차)**: MVP 시제품 완성 및 초기 유저 100명 유치 (전환율 5% 목표)
- **Phase 2 (6개월 차)**: 정식 서비스 유료 전환 ➔ 월 매출 목표 달성
- **Phase 3 (1년 차)**: B2B 파트너십 확장 ➔ 연 매출 돌파

### 3.3 정밀 자금 소요 및 조달 계획 (정부지원금 70% + 자부담금 30%)
| 사업비 집행 항목 | 세부 산출 근거 (수량 x 단가) | 금액 (원) | 정부지원금 | 자부담금 |
| :--- | :--- | :--- | :--- | :--- |
| **시제품 제작 및 개발/설비** | 로봇/시스템/인프라 구축비 | **35,000,000** | 25,000,000 | 10,000,000 |
| **마케팅 및 시장 진입비** | AI 타겟팅 SNS 광고 & SEO 마케팅 | **35,000,000** | 25,000,000 | 10,000,000 |
| **특허 출원 및 일반 운영비** | 지식재산권, 인허가, 연구 예비비 | **30,000,000** | 20,000,000 | 10,000,000 |
| **합 계** | **총 사업비 비중 100%** | **100,000,000** | **70,000,000** | **30,000,000** |

---

## 4. 📈 [성과 창출 & 팀 역량] (Performance & Team) - 추정 재무 수치
### 4.1 연차별 예상 매출 및 성과 추정표
| 연차 | 타겟 유저/매장 수 | 추정 월 매출 (원) | 추정 연 매출 (원) | 영업이익률 (%) |
| :--- | :--- | :--- | :--- | :--- |
| **1년 차 (2026년)** | 초기 거점 30개 / 1,000명 | **5,000,000** | **60,000,000** | **35%** |
| **2년 차 (2027년)** | 100개 매장 / 5,000명 | **20,000,000** | **240,000,000** | **45%** |
| **3년 차 (2028년)** | 전국 300개 매장 / 20,000명 | **50,000,000** | **600,000,000** | **55%** |

### 4.2 대표자 및 핵심 팀원의 직무 전문성 (신뢰도 수치화)
- **대표자 역량**: 본 아이템 관련 전공 및 관련 실무 경력 5년 이상 보유, 보유 특허/기술 자격 증명
- **외부 자문 및 파트너십**: 전문 연구기관 자문단 및 전담 제조/설비 협력사 MOU 체결 완료

---
*본 정밀 사업계획서는 중소벤처기업부 K-Startup PSST 공식 수치 검증 기준에 따라 "3분시리즈 1 AI 연구소"에서 정식 발급되었습니다.*"""

def call_openai_quantitative_gpt(req: ReportRequest, api_key: str) -> str:
    import json
...
[보고서 분류]: {req.category} (K-Startup 정밀 수치 검증 PSST 양식)
[타겟 고객층]: {req.target_customer}
[핵심 기능 및 경쟁력]: {req.core_features}
[사업비 및 예산]: {req.budget}

위 정보를 바탕으로 심사위원 합격 기준 95% 이상인 [문제인식 ➔ 해결방안 ➔ 실행전략 ➔ 성과창출] 4단계 정밀 수치 논리 검증 사업계획서를 마크다운으로 작성해 주세요."""

    data = {
        "model": "gpt-4o-mini",
        "messages": [
            {"role": "system", "content": QUANTITATIVE_PSST_SYSTEM_PROMPT.format(
...
def generate_business_report(req: ReportRequest) -> tuple[str, str]:
    ...
    if api_key:
        try:
            md_content = call_openai_quantitative_gpt(req, api_key)
            html_content = markdown.markdown(md_content, extensions=['tables', 'fenced_code'])
            return md_content, html_content
        except Exception as e:
            print(f"[WARN] OpenAI GPT Call failed: {e}, falling back to Quantitative PSST Engine")
    ...
    if domain == "fnb":
        ...
        comp_table = f"""| 구분 | 기존 수동 매장 / 일반 카페 | {req.title} (본 프로젝트) | 개선율 |
| :--- | :--- | :--- | :--- |
...
* **무인 로봇 디스플레이 사이니지 타겟 광고 수익**"""
        budget_table = f"""| 집행 항목 | 세부 내역 | 금액 (원) | 정부지원금 | 자부담금 |
| :--- | :--- | :--- | :--- | :--- |
| **로봇 제조/서빙 설비 및 키오스크** | 제조 로봇팔, 무인 서빙 로봇, 결제 키오스크 | **45,000,000** | 35,000,000 | 10,000,000 |
| **매장 인테리어 및 공간 구획** | 24시간 무인 매장 설계 및 시공 보증금 | **35,000,000** | 25,000,000 | 10,000,000 |
| **원자재 사입 및 바이럴 마케팅** | 원두/음료 부자재 및 개점 홍보 광고 | **20,000,000** | 10,000,000 | 10,000,000 |
...
| **3년 차 (2028년)** | 전국 가맹 50개점 돌파 | **150,000,000** | **1,800,000,000** | **55%** |"""
        ...
        comp_table = f"""| 구분 | 일반 소매 쇼핑몰 | {req.title} (본 프로젝트) | 개선율 |
| :--- | :--- | :--- | :--- |
...
| **상품 사입 및 풀필먼트 물류비** | 자동 물류 시스템 연동 및 초기 사입 | **30,000,000** | 20,000,000 | 10,000,000 |
| **쇼핑몰 시스템 및 디자인** | 쇼핑몰 구축 및 UX/UI 디벨롭 | **20,000,000** | 15,000,000 | 5,000,000 |
| **합 계** | **총 사업비 ({req.budget})** | **100,000,000** | **70,000,000** | **30,000,000** |"""
...
| **3년 차 (2028년)** | 월 20,000명 결제 유저 | **150,000,000** | **1,800,000,000** | **50%** |"""
        ...
        comp_table = f"""| 구분 | 기존 수동 서비스 / 대행사 | {req.title} (본 프로젝트) | 개선율 |
| :--- | :--- | :--- | :--- |
...
| **서버 인프라 및 시스템 구축** | 클라우드 서버, 보안, 기능 고도화 | **30,000,000** | 20,000,000 | 10,000,000 |
| **운영비 및 지식재산권** | 특허 출원, 인허가, 예비비 | **20,000,000** | 15,000,000 | 5,000,000 |
| **합 계** | **총 사업비 ({req.budget})** | **100,000,000** | **70,000,000** | **30,000,000** |"""
