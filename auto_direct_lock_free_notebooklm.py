...
def force_release_chrome_locks():
    print("🧹 대표님 Chrome의 모든 파일 핸들 락을 강제 정리합니다...")
    ...
    for lock_name in ["SingletonLock", "SingletonSocket", "SingletonCookie", "lockfile"]:
        lock_path = os.path.join(ORIGINAL_USER_DATA, lock_name)
        if os.path.exists(lock_path):
            try:
                os.remove(lock_path)
                print(f"🔓 락 파일 해제 완료: {lock_name}")
            except Exception:
                pass

async def run():
    print("🚀 [고감독 100% 자율 무인 완수] 대표님 원본 구글 계정으로 NotebookLM 자동 완성 가동!")
    
    with open(SRC_PATH, "r", encoding="utf-8") as f:
        content_text = f.read()

    force_release_chrome_locks()

    async with async_playwright() as p:
        try:
            print("🔑 대표님의 원본 구글 로그인 세션(User Data\\Default)을 100% 직접 오픈합니다...")
            context = await p.chromium.launch_persistent_context(
                user_data_dir=ORIGINAL_USER_DATA,
                channel="chrome",
                headless=False,
                args=[
            ...
            print("🌐 대표님 본인 구글 계정으로 NotebookLM 메인 접속 중...")
